...
} from "@livekit/components-react"; //from  "../../hooks";
...
>(function MediaDeviceSelect(
  ...
  return (
    <ul ref={ref} {...mergedProps} className="z-10 min-w-max">
      {devices.map((device, index) => (
        <li
          ...
          role="option"
          className="my-1 rounded-md bg-gray-900 p-1 text-white"
        >
          <button
            className={cn(
              "w-full rounded-md p-0.5 px-1 text-left",
              isActive(device.deviceId, activeDeviceId, index) && "bg-blue-950"
            )}
