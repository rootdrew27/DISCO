...
function validateTokenRequest(matchId: unknown, usernames: unknown): TokenRequest {
...
async function createDiscussionTokens(matchId: string, usernames: string[]): Promise<TokenResponse> {
  if (!matchId.trim()) {
  ...
  logger.info(`Successfully created ${Object.keys(tokens).length} tokens for match ${matchId}`);
  return tokens;
}
...
function expressErrorHandler(error: Error, _req: Request, res: Response, _next: NextFunction): void {
...
app.get("/tokens", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { matchId, usernames } = validateTokenRequest(req.query.matchId, req.query.usernames);
    const usernameArray = usernames.split(",").map(name => name.trim()).filter(name => name.length > 0);
    ...
    logger.info('Token request completed successfully', { matchId, userCount: usernameArray.length });
  } catch (error) {
    next(error);
  }
});
